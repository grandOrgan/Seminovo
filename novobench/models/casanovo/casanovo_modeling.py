...
def _initialize_beam_tokens(predictions: torch.Tensor, beam: int) -> torch.Tensor:
    """Seed every available first-token beam and pad unused slots."""
    batch = predictions.shape[0]
    available = min(int(beam), predictions.shape[-1])
    tokens = torch.zeros(
        batch,
        int(beam),
        dtype=torch.long,
    ...
    tokens[:, :available] = torch.topk(predictions[:, 0, :], available, dim=1)[1]
    return tokens


def _initial_inactive_beams(
    batch: int,
    beam: int,
    vocab: int,
    device: torch.device,
) -> torch.Tensor:
    """Mark fixed-width beam slots that have no unique first token."""
    available = min(int(beam), int(vocab))
    slots = torch.arange(int(beam), device=device).expand(int(batch), -1)
    return slots.ge(available).flatten()


def _mask_finished_beam_scores(
    scores: torch.Tensor,
    finished_beams: torch.Tensor,
    vocab: int,
) -> torch.Tensor:
    beam = finished_beams.numel() // scores.shape[0]
    active = (~finished_beams.reshape(scores.shape[0], beam)).repeat(1, vocab)
    active[:, :beam] = False
    return scores.masked_fill(~active, -torch.inf)
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    **kwargs : Dict
    ...
    def beam_search_decode(
        ...
        tokens[:, 0, :] = _initialize_beam_tokens(pred, beam)
        ...
        inactive_initial_beams = _initial_inactive_beams(
            batch, beam, vocab, tokens.device
        )
        ...
        for step in range(0, self.max_length):
            ...
            if step == 0:
                discarded_beams |= inactive_initial_beams
    ...
    def _cache_finished_beams(
        ...
        pred_cache: Dict[
            int, List[Tuple[float, Tuple[int, int], np.ndarray, torch.Tensor]]
        ],
    ):
        """
    ...
    def _get_topk_beams(
        ...
        ranked_scores = _mask_finished_beam_scores(
            step_scores.nanmean(dim=1), finished_beams, vocab
        )
        _, top_idx = torch.topk(ranked_scores, beam)
    ...
    def validation_step(
        ...
        if not self.saved_path == "":
            ...
            assert len(peptides_pred) == len(peptides_true) and len(
                peptides_score
            ) == len(peptides_true)
